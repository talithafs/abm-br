...
//				}
	...
	private void calculateReplacementInvestment() {
		...
		if(this.getNetWorth() > 0) {
			...
			double expn = 0.0000001*MetaParameters.getRepExponent() ;
			double prob = 1 - Math.exp(-expn*this.getNetWorth());
			int replace = 1 ;
...
//		}
	...
	public void receiveMachines() {
		...
		for(MachineOrder order : investments) {
			...
			if(replacement != null && replacement.getSecond().equals(newMachine)){
